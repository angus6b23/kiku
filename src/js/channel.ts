...
const channelInner = async (id:string, innertube: Innertube | null) => {
    ...
    try{
        ...
        const info = {
            ...
            subscribers: header.subscribers?.text?.replace(/ subscribers$/, '') as string,
            videoCount: Number(header.videos_count?.text?.replace(/ videos$/, '')) as number
        }
        const videoArr: (VideoResult | undefined)[] = res.videos.map((video) => {
            ...
            if (video.type === 'GridVideo'){
                return {
                    ...
                    videoThumbnails: extractInnertubeThumbnail(gridVideo.thumbnails),
                    viewCount: Number(gridVideo.views.text?.replace(/ views$/, '').replace(/,/g, '')),
                    lengthSeconds: toSecond(gridVideo.duration?.text as string)
                }
            } else {
                return undefined
            }
        })
        const videos = videoArr.filter(item => item !== undefined) as VideoResult[]
        return {
            channelInfo: info,
            videos: videos
        }
...
const channelInv = async(id:string, baseUrl: string) => {
    try{
        ...
        const videos: VideoResult[] = data.latestVideos.map((video: InvidiousRes) => {
            return {
                type: 'video',
                title: video.title,
...
const channelPiped = async(id:string, baseUrl: string) => {
    try{
        ...
        let videos: VideoResult[] = data.relatedStreams.map((video: PipedRes) => {
...
export async function fetchChannelDetails(id: string, instances: Instance[], innertube: Innertube | null): Promise<ChannelData | Error | undefined> {
    ...
    if (instances[0].enabled === false) {
        return await fetchChannelDetails(
            id,
            instances.slice(1),
            innertube
        )
