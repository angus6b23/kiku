...
import {Block, BlockTitle, Icon, Link, List, ListInput, ListItem, NavRight, Navbar, Page, f7} from "framework7-react"
...
export default function AccentColorModal(props: AccentColorModalProps): ReactElement {
    ...
    return <>
            ...
            <Block>
                ...
                <div className="flex justify-around items-center gap-4 flex-wrap">
                {
                    colors.map(color => {
                        ...
                        return (
                            <div key={color} className="h-20 w-20 rounded-md cursor-pointer" style={{backgroundColor: hex}} onClick={() => handlePrimaryColorChange(hex)}></div>
                        )
                    })
                }
                </div>
      <List strongIos outlineIos>
        <ListInput
          ...
          onColorPickerChange={(value) => handlePrimaryColorChange(value.hex)}
          colorPickerParams={{
